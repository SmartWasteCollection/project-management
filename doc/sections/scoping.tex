...
\begin{itemize}
    \item \textit{Ubiquitous Language} del \textbf{cittadino} (\refandback{img/citizen-ubiquitous-language}).
    \item \textit{Ubiquitous Language} della \textbf{raccolta} (\refandback{img/collection-ubiquitous-language}).
    \item \textit{Ubiquitous Language} del \textbf{cassonetto} (\refandback{img/dumpster-ubiquitous-language}).
    \item \textit{Ubiquitous Language} del \textbf{camioncino} (\refandback{img/truck-ubiquitous-language}).
    \item \textit{Ubiquitous Language} dei \textbf{rifiuti} (\refandback{img/waste-ubiquitous-language}).
\end{itemize}
...
\begin{itemize}
    \item \textbf{Ordinary waste disposal} (\refandback{uml/ordinary-disposal-use-cases}): lo scenario in cui il cittadino si reca presso il \textit{collection point} per conferire i propri rifiuti; gli attori coinvolti in tale caso d'uso sono il cittadino e il camioncino dei rifiuti.
    \item \textbf{Ordinary waste collection} (\refandback{uml/ordinary-collection-use-cases}): il processo che prevede la missione di raccolta dei rifiuti all'interno dei cassonetti da parte dei camioncini.
    \item \textbf{Extraordinary waste management} (\refandback{uml/extraordinary-management-use-cases}): la gestione dei rifiuti straordinari mediante raccolta a casa dei cittadini da parte dei camioncini.
    \item \textbf{Dashboard} (\refandback{uml/dashboard-use-cases}): l'utilizzo della dashboard da parte dei manager e della \textbf{Citizen App} da parte dei cittadini.
    \item \textbf{Complaints} (\refandback{uml/complaints-use-cases}): la presentazione di reclami da parte di cittadini, autisti dei camioncini e cassonetti e successiva visualizzazione da parte dei manager.
\end{itemize}

\subsubsection{Requirement Breakdown Structure}\label{sec:rbs}
Una volta analizzati i casi d'uso e le \textit{user stories}, è stato prodotto un diagramma di \textit{Requirement Breakdown Structure} \refandback{uml/requirement-breakdown-structure}. Questo permette di descrivere ogni requisito in termini delle funzionalità che questo introduce.
...
\possec{Criteri di Successo}{
    \begin{enumerate}
        \item I cassonetti dei rifiuti non raggiungono mai il 100\% di volume occupato.
        \item Non si ricevono più di 5 reclami al mese.
        \item Lo smaltimento di rifiuti straordinari avviene al 95\% in seguito a prenotazioni sulla \textbf{Citizen App}.
    \end{enumerate}
}
